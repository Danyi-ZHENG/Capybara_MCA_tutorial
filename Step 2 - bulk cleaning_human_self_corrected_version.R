...
for (tis in tissue.uniq) {
  ...
  if (nrow(curr.geo.to.keep) >= n.sample) {
    geo.sample <- get.most.connected(log2(raw.counts[,curr.geo.to.keep$accession] + 1), n.sample = n.sample)
  } else {
...
pheatmap::pheatmap(corr.sample.rpkm, color = viridis(20, option = "A"), show_colnames = F, show_rownames = F, cluster_cols = F, cluster_rows = F, clustering_method = "ward.D2", 
                   annotation_col = my.col, cellheight = 0.2, cellwidth = 0.2, file = "test.pdf")
pheatmap::pheatmap(corr.sample.rpkm.log, color = viridis(20, option = "A"), show_colnames = F, show_rownames = F, cluster_cols = F, cluster_rows = F, clustering_method = "ward.D2", 
                   annotation_col = my.col, cellheight = 0.2, cellwidth = 0.2, file = "test_log.pdf")


### Look at the correlation between samples
corr.sample.count <- WGCNA::cor(sample.count)
corr.sample.count.log <- WGCNA::cor(log2(sample.count + 1))

my.col <- data.frame(row.names = rownames(corr.sample.count), 
                     tissue = unlist(lapply(strsplit(rownames(corr.sample.count), "_"), function(x) x[1])), 
                     stringsAsFactors = F)

### Heatmap plot to see if the samples selected are well correlated as well as well distinguished from other tissues
pheatmap::pheatmap(corr.sample.count, color = viridis(20, option = "A"), show_colnames = F, show_rownames = F, cluster_cols = F, cluster_rows = F, clustering_method = "ward.D2", 
                   annotation_col = my.col, cellheight = 0.2, cellwidth = 0.2, file = "test_count.pdf")
pheatmap::pheatmap(corr.sample.count.log, color = viridis(20, option = "A"), show_colnames = F, show_rownames = F, cluster_cols = F, cluster_rows = F, clustering_method = "ward.D2", 
                   annotation_col = my.col, cellheight = 0.2, cellwidth = 0.2, file = "test_log_count.pdf")

saveRDS(sample.count, "05_90_sampled_raw_counts.Rds")
saveRDS(sample.rpkm, "05_90_sampled_raw_rpkm.Rds")
...
pheatmap::pheatmap(rpkm.tissue.cor, color = viridis(20, option = "A"), show_colnames = T, show_rownames = T, cluster_cols = F, cluster_rows = F, clustering_method = "ward.D2", 
                   cellheight = 15, cellwidth = 15, file = "test_log.pdf")

### Check if the averaged bulk dataset has tissues that are distinguishable from each other
raw.count.tissue.cor <- WGCNA::cor(log2(final.raw.count.tissue + 1))
pheatmap::pheatmap(raw.count.tissue.cor, color = viridis(20, option = "A"), show_colnames = T, show_rownames = T, cluster_cols = F, cluster_rows = F, clustering_method = "ward.D2", 
                   cellheight = 15, cellwidth = 15, file = "test_log_count.pdf")

saveRDS(final.rpkm.tissue, "05_90_sample_final_rpkm_tissue_collapsed.Rds")
saveRDS(final.raw.count.tissue, "05_90_sample_final_raw_count_tissue_collapsed.Rds")
